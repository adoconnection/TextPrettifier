...
namespace TextPrettifierLib
{
    ...
    {
        public static Regex htmlSplitterRegex = new Regex(@"^[^<>]+|(?<=>)[^<>]+(?=<)|[^<>]+$", RegexOptions.Singleline | RegexOptions.Compiled);
        ...
        public static IDictionary<Regex, MatchEvaluator> WebRules = new Dictionary<Regex, MatchEvaluator>()
        {
                ...
                { new Regex(@"(\s+)(?<g1>[a-zа-я]{1,3})(\s+)(?<g2>[a-zа-я\.]{4,})", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase), match => " " + match.Groups["g1"].Value + "&nbsp;" + match.Groups["g2"].Value },
                ...
                // non-breaking space between short words if any
                { new Regex(@"(\s+|^)(?<g1>[a-zа-я]{1,2})(\s+)(?<g2>[a-zа-я\.]{3})(?<g3>[\s\.,]+|$)", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase), match => " " + match.Groups["g1"].Value + "&nbsp;" + match.Groups["g2"].Value + match.Groups["g3"].Value },
        ...
        {
            return htmlSplitterRegex.Replace(
                    text, 
                    match => this.Prettify(WebRules, match.Value));
        }
